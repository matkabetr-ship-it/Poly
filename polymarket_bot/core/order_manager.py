...
class OrderManager:
    def __init__(self, cfg, exchange):
        ...
        self.open_orders: dict[str, tuple[str, float]] = {}

    async def submit(self, intent: OrderIntent, mark_price: float) -> str | None:
        ...
        self.open_orders[oid] = (intent.market_id, time.time())
        return oid

    async def cancel_stale(self) -> None:
        now = time.time()
        stale = [oid for oid, (_, ts) in self.open_orders.items() if now - ts > self.cfg.order_ttl_seconds]
        for oid in stale:
